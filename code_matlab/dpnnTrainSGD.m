function [model, err, perf] = dpnnTrainSGD( model, x, t, batch_size, step_size, annealing_rate, momentum, n_epochs, varargin )
% Performs stochastic gradient descent with momentum to optimize the
% weights of a DP neural network. Optimization is stopped prematurely if
...
elseif strcmp(model.sharing, 'global')
  delta_W = zeros(size(model.W));
else
  error('Error in ''dpnnTrainSGD'': Unrecognized sharing ''%s''', model.sharing);
end
f = dpnnErrorFunction(model, x{1}, t{1});

if do_output
  fprintf('dpnnTrainSGD\n');
  ...
  for it_n = 1:batch_size:N_tr
    ...
    if strcmp(model.sharing, 'layerwise')
      for l = 1:model.num_layers
        ...
        delta_W{l} = momentum * delta_W{l} - step_size * grad_batch{l};
    ...
    elseif strcmp(model.sharing, 'global')
      ...
      delta_W = momentum * delta_W - step_size * grad_batch;
