function err = dpnnEvaluate( model, x, t )
% Evaluates the given DP neural network and returns the performance on the
% given data.
%
% Input:
...
elseif strcmp(model.task, 'muclass')
  ...
  error('Error in ''dpnnEvaluate'': Unrecognized task ''%s''', model.task);
end
err = gather(err);
