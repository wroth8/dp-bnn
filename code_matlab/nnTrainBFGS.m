function [model, err, perf, fmincon_output] = nnTrainBFGS( model, x, t, n_iterations, varargin )
% Performs quasi-Newton optimization of the weights of a plain feed-forward
% neural network with the Matlab builtin function fmincon. In case the
% number of weights to optimize a larger than 1024 the memory limited
% L-BFGS method is used. Otherwise the full BFGS algorithm is used.
%
% Input:
% model: The DP neural network (see nnInit)
...
%  nnEvaluate). In case more than a single dataset is given, i.e. x and t
...
if length(varargin) > 1
  error('Error in ''nnTrainBFGS'': Too much arguments given');
...
if (iscell(x) && ~iscell(t)) || (~iscell(x) && iscell(t))
  error('Error in ''nnTrainBFGS'': Either both ''x'' and ''t'' are cell arrays or none');
end
...
for it = 1:numel(x)
  ...
  if size(x{it}, 1) ~= size(t{it}, 1)
    error('Error in ''nnTrainBFGS'': ''x{%d}'' and ''t{%d}'' must contain same number of rows', it, it);
  end
  if size(x{it}, 2) ~= model.layout(1)
    error('Error in ''nnTrainBFGS'': Number of columns in ''x{%d}'' are inconsistent with the given model', it);
  end
  if strcmp(model.task, 'muclass') && size(t{it}, 2) ~= 1
    error('Error in ''nnTrainBFGS'': Number of columns in ''t{%d}'' must be 1 in case of multiclass classification', it);
  elseif ~strcmp(model.task, 'muclass') && size(t{it}, 2) ~= model.layout(end)
    error('Error in ''nnTrainBFGS'': Number of columns in ''t{%d}'' are inconsistent with the given model', it);
  end
end
...
w0 = zeros(1, model.num_unique_weights);
start_idx = 1;
for l = 1:model.num_layers
  ...
  w0(start_idx:end_idx) = model.b{l}(:);
  start_idx = end_idx + 1;
end
...
[~,~,~,fmincon_output] = fmincon(@objfun, w0, [], [], [], [], ones(size(w0)) * (-inf), [], [], options);
  ...
  function stop = outfun(vec_nn, optim_values, state)
    ...
    for l = 1:model.num_layers
      end_idx = start_idx + model.layout(l) * model.layout(l+1) - 1;
      model.W{l} = reshape(vec_nn(start_idx:end_idx), [model.layout(l), model.layout(l+1)]);
      start_idx = end_idx + 1;
      end_idx = start_idx + model.layout(l+1) - 1;
      model.b{l} = reshape(vec_nn(start_idx:end_idx), [1, model.layout(l+1)]);
      start_idx = end_idx + 1;
    end
    
    err(1, optim_values.iteration) = optim_values.fval;
    perf(1, optim_values.iteration) = nnEvaluate(model, x{1}, t{1});
    for it_x = 2:numel(x)
      err(it_x, optim_values.iteration) = nnErrorFunction(model, x{it_x}, t{it_x});
      perf(it_x, optim_values.iteration) = nnEvaluate(model, x{it_x}, t{it_x});
    end
  ...
  function [f, g] = objfun(vec_nn)
    ...
    [g_W, g_b, f] = nnErrorGradient(model, x{1}, t{1}, true);
    ...
    for l = 1:model.num_layers
      end_idx = start_idx + model.layout(l) * model.layout(l+1) - 1;
      g(start_idx:end_idx) = g_W{l}(:);
      start_idx = end_idx + 1;
      end_idx = start_idx + model.layout(l+1) - 1;
      g(start_idx:end_idx) = g_b{l}(:);
      start_idx = end_idx + 1;
    end
  end
